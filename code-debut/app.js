...
var pantalons = [pantalonA, pantalonB, pantalonC, pantalonD];
...
for (var i = 0; i < couleurs.length; i = i + 1) {
  $("#boutonsCouleur").append(
    '<button type="button" class="btn btn-filtre-couleur btn-' +
      couleurs[i] +
      '" data-couleur="' +
      i +
      '"></button>'
...
for (var i = 0; i < tailles.length; i++) {
  ...
  var classeBouton = isTailleDispo
    ? "btn-outline-dark"
    : "btn-outline-secondary unavailable"; // Ajout de la classe "unavailable" si la taille n'est pas disponible

  var contenuBoutonTaille =
    '<button type="button" class="btn ' +
    classeBouton +
    " btn-filtre-taille btn-taille-" +
    tailles[i] +
    '" data-taille="' +
    tailles[i] +
    '">' +
...
$(document).on("click", ".btn-filtre-taille", function () {
  ...
  for (var i = 0; i < couleurs.length; i++) {
    ...
    for (var j = 0; j < pantalons.length; j++) {
      if (
        pantalons[j]["couleur"] === couleurs[i] &&
        pantalons[j]["taillesDispo"].includes(parseInt(tailleId))
      ) {
        isCouleurDispo = true;
        break;
      }
    ...
    $(".btn-filtre-couleur.btn-" + couleurs[i]).removeClass(
      "btn-border-select btn-border-none"
    );
...
function ajouterAuPanier(couleur, taille) {
  ...
  console.log(
    "Article ajouté au panier : Couleur - " + couleur + ", Taille - " + taille
  );
...
$(".btn-filtre-couleur").on("click", function () {
  ...
  contenuImage =
    contenuImage + 'alt="' + pantalons[couleurId]["imageAlt"] + '"';
  contenuImage = contenuImage + 'class="img-fluid">';
  $("#image-produit").append(contenuImage);
  ...
  for (var i = 0; i < tailles.length; i++) {
    var isTailleDispo = pantalons[couleurId]["taillesDispo"].includes(
      tailles[i]
    );
    var classeBouton = isTailleDispo
      ? "btn-outline-dark"
  ...
  $(document).on("click", ".btn-filtre-taille", function () {
    ...
    for (var i = 0; i < couleurs.length; i++) {
      ...
      var classeBouton = isCouleurDispo
        ? "btn-border-select"
        : "btn-border-none";
  ...
  $(".btn-filtre-couleur").on("click", function () {
    ...
    // Réinitialiser les bordures des boutons couleur
    $(".btn-filtre-couleur").removeClass(
      "btn-border-select btn-border-none active"
...
$("#bouton-ajout").on("click", function () {
  ...
  if (tailleId === "") {
    $(".optionTailleSelectionee").html(
      '<div class="alert alert-warning" role="alert"> Veuillez sélectionner une taille.</div>'
    );
  }
  // Taille sélectionnée, ajouter au panier
  else {
    console.log(
      "Produit ajouté au panier : Couleur - " +
        couleurs[couleurId] +
        ", Taille - " +
        tailleId
    );
